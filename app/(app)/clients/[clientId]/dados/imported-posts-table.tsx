...
export function ImportedPostsTable({ posts }: { posts: EditorialPost[] }) {
  ...
  const platforms = useMemo(() => {
    ...
    for (const post of posts) values.add(post.social_accounts.platform);
  ...
  const filtered = useMemo(() => {
    ...
    return posts.filter((post) => {
      ...
      if (platform !== "all" && post.social_accounts.platform !== platform) {
        return false;
      }
  ...
  }, [posts, search, platform, postType, dateFrom, dateTo]);
  ...
  if (posts.length === 0) {
    return (
      <p className="text-sm text-zinc-500 dark:text-zinc-400">
        Nenhum post importado ainda — faça upload de um relatório na aba
        &quot;Contas &amp; Upload&quot;.
      </p>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-lg border border-black/10 bg-white p-3 dark:border-white/10 dark:bg-zinc-900">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs text-zinc-500 dark:text-zinc-400">Rede social</label>
          <select
            value={platform}
            onChange={(e) => {
            ...
            className="rounded-md border border-zinc-300 px-2 py-1.5 text-sm capitalize dark:border-zinc-700 dark:bg-zinc-950"
          >
